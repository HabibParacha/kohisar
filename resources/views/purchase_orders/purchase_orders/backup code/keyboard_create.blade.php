...
<script>
    ...
    // Additionally, you may want to handle focusing after the Select2 dropdown is closed
    $(document).on('select2:close', '#party_id', function() {
        $('#vehicle_no').focus(); // Move to the next input when the dropdown closes
    });
        ...
        if (e.key === 'Enter') {
            ...
            if ($(this).hasClass('item-dropdown-list')) {
                ...
                if ($row.next('tr').length === 0) {
                    appendNewRow(); // Call the function if there is no next row
                } else {
                    $row.next('tr').find('.item-dropdown-list').focus().select2('open'); // Move to the next row's input if it exists
    ...
    function appendNewRow(){
        ...
        let row = `
             <tr>
                ...
                <td> 
                    <input type="number" name="cut_percentage[]"  step="0.01" class="form-control item-cut-percentage d-none">  
                </td>
                <td> 
                ...
                <td> 
                    <input type="number" name="total_quantity[]" step="0.01" class="form-control item-total-quantity">  
                </td>
                <td> 
                    <input type="number" name="per_package_weight[]" step="0.01" class="form-control item-per-package-weight">  
                </td>
                <td> 
