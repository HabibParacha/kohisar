                            ...
                            <div class="row mt-3">
                                ...
                                <div class="col-md-4 d-flex align-items-center">
                                    <table id="summary-table" class="table">
                                        ...
                                        <tr>
                                            <th width="50%">Sub Total Stock</th>
                                            <td width="50%">
                                        ...
                                        <tr>
                                            <th>Freight </th>
                                            <td>
                                                <input type="hidden" name="xfreight" id="x-freight-value" value="0" readonly>
                                                <div class="row">
...
<script>  
    ...
    function summaryCalculation()
    {
        ...
        let total_freight = parseFloat($('#total-freight').val()) || 0;
        let total_after_cut_weight = 0;
        let xfreight_per_kg = 0;// this will be added to per kg price supplier if x-freight is enabled

        $('.item-after-cut-total-weight').each(function(){
            total_after_cut_weight += parseFloat($(this).val()) || 0;
        });
        ...
        if(x_freight_checkbox && total_freight > 0){
            xfreight_per_kg = total_freight/total_after_cut_weight;
        ...
        $('.item-per-unit-price').each(function(){
            let row = $(this).closest('tr');

            let unit_price = parseFloat($(this).val());
            let after_cut_wgt = parseFloat(row.find('.item-after-cut-total-weight').val());

            let stock_unit_price = parseFloat(unit_price + xfreight_per_kg);
            row.find('.item-per-unit-price-stock').val(stock_unit_price.toFixed(2));


            let stock_total_price = parseFloat(stock_unit_price * after_cut_wgt);
    ...
    function toggleXFreightValue(){
        ...
        if(xfreightCheckbox){
            $('#x-freight-value').val('1');
        }else{
            $('#x-freight-value').val('0');
        }
    }
